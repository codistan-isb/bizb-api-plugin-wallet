...
export default async function createWallet(parent, args, context, info) {
  ...
  if (!args.userId || !args.amount || !args.paymentProcessor) {
    throw new ReactionError("Invalid Input",'Invalid input. Please provide userId, amount, and paymentProcessor.');
  }
  ...
  if (existingWallet) {
    ...
    throw new ReactionError("wallet exist",'Wallet already exists for this user.');
